                                ...
                                sandbox=TRUE)
{
  ...
  if(resultSet == "incomplete"){
    ...
    #Get assignments
    results <- MTurkR::GetAssignments(hit = HITSet$HITId, sandbox=sandbox)
                            ...
                            confirm = TRUE)
{
  ...
  if(!is.null(HITSet)){
    if(class(HITSet) != "data.frame") stop("HITSet must be a data frame with 'HITTypeId', 'WorkerId', and 'AssignmentId' columns.")
    if(!all(c("HITTypeId","workerIds","assignmentIds")))
  ...
  if(existingBonuses == "No existing bonuses"){
    ...
    if(verbose) print(existingBonuses)
